<?php
	 ...
	 * listing a collection of Samples objects.  It includes
	 ...
	 *
	 */
	class SamplesDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Samples datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Samples object, or it can be any QQNode extending from QQN::Samples().
		 *
		 ...
		 * sorting using two fields from the Samples object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Samples
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Samples::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Samples, given the clauses above
			$this->DataSource = Samples::QueryArray($objConditions, $objClauses);
		}

		/**
		 * Used internally by the Meta-based Add Column tools.
		 *
		 * Given a QQNode or a Text String, this will return a Samples-based QQNode.
		 * It will also verify that it is a proper Samples-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'samples') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "samples".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Samples()->Id;
				case 'Guid': return QQN::Samples()->Guid;
				case 'UserName': return QQN::Samples()->UserName;
				case 'SubmitDate': return QQN::Samples()->SubmitDate;
				case 'Email': return QQN::Samples()->Email;
				case 'SpeakText': return QQN::Samples()->SpeakText;
				case 'Translation': return QQN::Samples()->Translation;
				default: throw new QCallerException('Simple Property not found in SamplesDataGrid content: ' . $mixContent);
