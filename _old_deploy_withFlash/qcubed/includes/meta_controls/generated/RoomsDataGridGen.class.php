<?php
	 ...
	 * listing a collection of Rooms objects.  It includes
	 ...
	 *
	 */
	class RoomsDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Rooms datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Rooms object, or it can be any QQNode extending from QQN::Rooms().
		 *
		 ...
		 * sorting using two fields from the Rooms object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Rooms
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Rooms::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Rooms, given the clauses above
			$this->DataSource = Rooms::QueryArray($objConditions, $objClauses);
		}

		/**
		 * Used internally by the Meta-based Add Column tools.
		 *
		 * Given a QQNode or a Text String, this will return a Rooms-based QQNode.
		 * It will also verify that it is a proper Rooms-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'rooms') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "rooms".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Rooms()->Id;
				case 'HostID': return QQN::Rooms()->HostID;
				case 'UnlockedTracks': return QQN::Rooms()->UnlockedTracks;
				case 'CreatedTime': return QQN::Rooms()->CreatedTime;
				case 'Urk': return QQN::Rooms()->Urk;
				default: throw new QCallerException('Simple Property not found in RoomsDataGrid content: ' . $mixContent);
