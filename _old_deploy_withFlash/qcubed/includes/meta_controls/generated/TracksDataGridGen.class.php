<?php
	 ...
	 * listing a collection of Tracks objects.  It includes
	 ...
	 *
	 */
	class TracksDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Tracks datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Tracks object, or it can be any QQNode extending from QQN::Tracks().
		 *
		 ...
		 * sorting using two fields from the Tracks object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Tracks
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Tracks::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Tracks, given the clauses above
			$this->DataSource = Tracks::QueryArray($objConditions, $objClauses);
		}

		/**
		 * Used internally by the Meta-based Add Column tools.
		 *
		 * Given a QQNode or a Text String, this will return a Tracks-based QQNode.
		 * It will also verify that it is a proper Tracks-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'tracks') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "tracks".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Tracks()->Id;
				case 'RoomId': return QQN::Tracks()->RoomId;
				case 'UserId': return QQN::Tracks()->UserId;
				case 'TrackNumber': return QQN::Tracks()->TrackNumber;
				default: throw new QCallerException('Simple Property not found in TracksDataGrid content: ' . $mixContent);
