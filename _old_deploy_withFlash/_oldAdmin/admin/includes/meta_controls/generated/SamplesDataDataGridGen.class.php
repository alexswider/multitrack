<?php
	 ...
	 * listing a collection of SamplesData objects.  It includes
	 ...
	 *
	 */
	class SamplesDataDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this SamplesData datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the SamplesData object, or it can be any QQNode extending from QQN::SamplesData().
		 *
		 ...
		 * sorting using two fields from the SamplesData object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from SamplesData
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = SamplesData::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from SamplesData, given the clauses above
			$this->DataSource = SamplesData::QueryArray($objConditions, $objClauses);
		}

		/**
		 * Used internally by the Meta-based Add Column tools.
		 *
		 * Given a QQNode or a Text String, this will return a SamplesData-based QQNode.
		 * It will also verify that it is a proper SamplesData-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'samplesData') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "samplesData".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::SamplesData()->Id;
				case 'Guid': return QQN::SamplesData()->Guid;
				case 'UserName': return QQN::SamplesData()->UserName;
				case 'SubmitDate': return QQN::SamplesData()->SubmitDate;
				case 'Email': return QQN::SamplesData()->Email;
				case 'SpeakText': return QQN::SamplesData()->SpeakText;
				case 'Translation': return QQN::SamplesData()->Translation;
				case 'Newsletter': return QQN::SamplesData()->Newsletter;
				case 'Score': return QQN::SamplesData()->Score;
				default: throw new QCallerException('Simple Property not found in SamplesDataDataGrid content: ' . $mixContent);
